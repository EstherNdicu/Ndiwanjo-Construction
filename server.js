  ...
  try {
    const projects = await prisma.project.findMany({
      ...
      include: {
        ...
        expenses: true,
        employees: true,
        customer: true, // Ã¢Å“â€¦ include linked customer
      },
    })
    res.json(projects)
  } catch (err) {
  ...
  try {
    ...
    const project = await prisma.project.create({
      data: {
        ...
        customerId: customerId ? Number(customerId) : null, // Ã¢Å“â€¦
      },
    })
...
app.get('/customers', async (req, res) => {
  try {
    const customers = await prisma.customer.findMany({
      orderBy: { name: 'asc' },
      include: { projects: true }, // Ã¢Å“â€¦ include linked projects
    })
    res.json(customers)
  } catch (err) {
...
app.listen(PORT, () => {
  console.log(`Ã¢Å“â€¦ Ndiwanjo server running on http://localhost:${PORT}`)
})