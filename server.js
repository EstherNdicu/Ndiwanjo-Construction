...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    const dir = path.join(UPLOADS_DIR, 'project_' + req.params.id)
...
app.get('/projects/:id/documents', (req, res) => {
  const dir = path.join(UPLOADS_DIR, 'project_' + req.params.id)
  ...
  const files = fs.readdirSync(dir).map(filename => {
    ...
    return {
      ...
      url: '/uploads/project_' + req.params.id + '/' + filename,
...
app.post('/projects/:id/documents', upload.single('file'), async (req, res) => {
  if (!req.file) return res.status(400).json({ error: 'No file uploaded' })
  const originalName = req.body.originalName || req.file.originalname
  await logActivity('Document uploaded to project', originalName)
  res.json({
    filename: req.file.filename,
    originalName,
    size: req.file.size,
    uploadedAt: new Date(),
    url: '/uploads/project_' + req.params.id + '/' + req.file.filename,
    category: detectCategory(originalName)
  })
})

app.delete('/projects/:id/documents/:filename', (req, res) => {
  const filepath = path.join(UPLOADS_DIR, 'project_' + req.params.id, req.params.filename)
